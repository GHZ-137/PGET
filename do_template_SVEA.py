...
def template_SVEA(r, n_rows, n_cols, delta, scale = 1, disp_x = 0, disp_y = 0):
    res = []
    for rr in range(n_rows * 2 + 1):
...
dxs = np.linspace(40, 70, n_intervals *2, endpoint = True, dtype = np.float)
dys = np.linspace(40, 70, n_intervals *1, endpoint = True, dtype = np.float)
    ...
    for s in ss:
        for dx in dxs:
            for dy in dxs:
                ...
                blobs = template_SVEA(r, n_rows, n_cols, delta, s, dx, dy)
